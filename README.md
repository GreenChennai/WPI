...
- **动画渲染**：GIF / MP4 录制前先触发 reveal-on-scroll 内容展开（整页可见，不冻结动画），并**录满用户设定的动画时长上限**（如 15s）。GIF 按目标帧率重采样——总帧数 = 帧率 × 时长（25fps × 15s = 375 帧）、单帧延迟 = 100 ÷ 帧率 百分秒（25fps → 4cs = 40ms）；MP4 同样补齐到帧率 × 时长（30fps × 15s = 450 帧）后按设定帧率编码——播放时长与设定一致、动画速度 = 真实时间。PNG / PDF 在导出前等待字体 / 图片就绪并将有限动画锁定为「播放完毕」状态，无限循环动画则等待其完全展开（默认 3s）后再截取。
...
- **GIF / MP4 参数**：帧速严格限 4 档——10 / 20 / 25 / 50 fps（对应 GIF 单帧延迟 10 / 5 / 4 / 2 百分秒 = 100 / 50 / 40 / 20 ms，延迟恒为整数不产生 BUG）；「无限循环」开关默认勾选（垂直左对齐），取消勾选后才显示次数输入窗口（最小 1）；所有数字输入框已去除 +/- 步进按钮，界面更清爽。视频录制采用自适应节奏（截图耗时超过 1/fps 时不再 sleep，最大化采样率）减少帧重复，动画播放更流畅、速度 = 真实时间。
