...
- **四种输出格式**：
  ...
  - **MP4** — 与 GIF 同一套整页帧序列，FFmpeg 编码为 **H.264 高保真视频**（crf 0 + yuv444p，逐帧采样走 CDP JPEG 质量 95 视觉无损提速，随包附送 ffmpeg），适合二次剪辑后处理。
  - **PDF** — 整页排版输出，按**屏幕样式**渲染（所见即所得，避免网页打印样式导致全白）；超长内容自动**分页**为常规尺寸页面（单页高度上限 2400px），Edge / Chrome 等浏览器均可正常查看。
- **在线网站导出**：导出尺寸下方新增「在线网站」地址框，输入 URL 后可一键「预览」、在系统浏览器中打开，或直接导出（支持 PNG / GIF / MP4 / PDF 全部格式），与本地项目流程完全一致。
- **动画渲染**：GIF / MP4 录制前先触发 reveal-on-scroll 内容展开（整页可见，不冻结动画），并**录满用户设定的动画时长上限**（如 15s）。逐帧采样走 **CDP JPEG 直采**（质量 95 视觉无损，JPEG 编码/解码远快于 PNG，整页采样率提升数倍）。**播放速度恒等于真实时间**：采样节奏达到设定帧率时按设定帧率编码；整页截图太慢导致采样率不足时，按真实采集节奏逐帧播放（每帧延迟 = 真实采集间隔），绝不用帧重复把动画拖慢。GIF 单帧延迟取整到百分秒（限幅 20~1000ms），MP4 以实际采样帧率编码。PNG / PDF 在导出前等待字体 / 图片就绪并将有限动画锁定为「播放完毕」状态，无限循环动画则等待其完全展开（默认 3s）后再截取。
...
- **分辨率倍率（原生放大）**：「导出尺寸」下新增 X1 / X2 / X4 / X8 分辨率倍率下拉。页面仍按设定宽度布局（比例与排布完全不变），但导出分辨率按倍率原生放大——底层使用浏览器的 `deviceScaleFactor` 高 DPI 渲染（如 1080px × X2 = 2160px 宽输出），是真实矢量级高清，而非超分插值。4X / 8X 整页导出：尺寸未超单拍上限时直接单拍（fixed/sticky 元素只画一次、无拼接）；超限时用「滚动触发栅格化 + 视口相对 clip 分块」拼接（v2.3.0 起）。分块时块间等待 400ms + 2 个 rAF 帧确保 canvas/JS 内容绘制完整，并**只对「顶部小条」类 fixed 元素去重隐藏**——底部装饰、侧边标签等固定组件正常保留（v2.6.0），不再出现卡片内容缺失。
- **单端口静态服务**：预览与「浏览器打开」共用进程内唯一的本地静态服务（整个软件只占一个端口）；在「工作目录」中切换项目时只需切换该服务的挂载目录，URL 自动带唯一参数并禁用缓存，保证切换后一定加载新项目，不再为每个预览/打开新开端口。
- **GIF / MP4 参数**：帧速分档设置——**GIF 限 4 档 10 / 20 / 25 / 50 fps**（对应 GIF 单帧延迟 10 / 5 / 4 / 2 百分秒 = 100 / 50 / 40 / 20 ms，延迟恒为整数不产生 BUG）；**MP4 视频另设 24 / 30 / 48 / 60 fps**（视频编码无整数延迟约束，采样率不足时按真实采样节奏编码）。切换导出格式时帧率预设自动切换。「无限循环」开关默认勾选（垂直左对齐），取消勾选后才显示次数输入窗口（最小 1）；所有数字输入框已去除 +/- 步进按钮，界面更清爽。视频录制采用自适应节奏（截图耗时超过 1/fps 时不再 sleep，最大化采样率）+ 真实时间播放，动画更流畅、绝不被重复帧拖慢。
