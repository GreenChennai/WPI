...
- **四种输出格式**：
  ...
  - **GIF** — 滚动逐帧录制整页动画（含转场、轮播、Hover、reveal-on-scroll 入场动画），可选帧率与循环次数。
  - **MP4** — 与 GIF 同一套滚动录制帧序列，FFmpeg 编码为 H.264 视频（随包附送 ffmpeg）。
...
- **动画渲染**：GIF / MP4 通过「滚动逐帧录制」捕获整页——从顶部开始缓慢滚动到底，自然触发 reveal-on-scroll 入场动画并覆盖全部内容，解决「只录到顶部标题、下方是背景色块」的问题；PNG / PDF 在导出前等待字体 / 图片就绪并将有限动画锁定为「播放完毕」状态，无限循环动画则等待其完全展开（默认 3s）后再截取。
...
- **GIF / MP4 参数**：帧率下拉可选 15 / 24 / 30 / 48 / 60 fps；循环次数带「无限循环」开关（默认无限循环，关闭后可填 ≥1 的次数）；所有数字输入框已去除 +/- 步进按钮，界面更清爽。
...
4. **设置尺寸**：输入或选择导出宽度（高度跟随网页内容自动计算）。如需导出在线网站，在「在线网站」地址框输入 URL，可先点「预览」或「浏览器打开」确认。
...
### v2.0.0
- **无 GUI 导出（命令行）**：`WPI.exe --export --source <路径或URL> --output out.png --format PNG --width 1080` 支持纯命令行导出本地项目与在线网站；打包后的 windowed 主程序会自动挂接父控制台显示进度与结果。
- **下拉箭头图标重做**：此前 base64 data URI 在 Qt QSS `image:url()` 中不可靠导致箭头空白；改为随包携带真实 PNG 箭头资源（`src/gui/assets/down_arrow*.png`，spec 已打包），普通下拉框与项目卡片入口下拉框均恢复箭头。
- **批量导出重名保护**：目标文件已存在时自动追加 `_1/_2…` 数字后缀，绝不覆盖同名文件。
- **完成提示区分**：单选导出即使走批量通道，也只显示「导出完成」，不再误弹「批量导出完成」。
- **单实例运行**：软件只允许打开一个实例（QLockFile），重复启动提示「WPI 已在运行中」。
- **GIF 滚动录制**：GIF 改为从头到尾滚动逐帧录制整页，自然触发 reveal-on-scroll 入场动画并覆盖全部内容，解决「只录到顶部标题、下方是背景色块」的问题。
- **新增 MP4 导出**：与 GIF 共用滚动录制帧序列，FFmpeg 编码为 H.264 / yuv420p / faststart 的 MP4（构建随包附送 ffmpeg.exe）。
- **GIF 参数升级**：帧率下拉可选 15 / 24 / 30 / 48 / 60 fps（上限由 30 提到 60）；循环次数改为「无限循环」开关（默认开启），关闭后可填 ≥1 的次数；所有数字输入框移除 +/- 步进按钮。
- **在线网站导出**：导出尺寸下方新增「在线网站」地址框 + 「预览」「浏览器打开」按钮；填入 URL 后可直接预览 / 浏览器打开 / 导出（全格式支持），导出时优先于本地项目。
- **构建**：归档于 `构建\WPI-v2.0.0\`（含 WPI.exe + ffmpeg.exe + README + VERSION.txt）。
